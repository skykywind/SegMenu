//
//  SegMenu+UIGestureDelegate.swift
//  FBSnapshotTestCase
//
...
extension SegMenu: UIGestureRecognizerDelegate {
    @objc func handleMenuItemTap(_ gestureRecognizer : UITapGestureRecognizer) {
        ...
        if tappedPoint.y < menuScrollView.frame.height {
            ...
            if itemIndex >= 0 && itemIndex < controllerArray.count {
                ...
                let timerInterval : TimeInterval = Double(configuration.scrollDuration) * 0.001
                tapTimer = Timer.scheduledTimer(timeInterval: timerInterval, target: self, selector: #selector(SegMenu.scrollViewDidEndTapScrollingAnimation), userInfo: nil, repeats: false)
            }
        }
    }
}
