...
@objc public protocol SegMenuDelegate {
...
open class SegMenu: UIViewController {
    
    //MARK: - Configuration
    var configuration = SegMenuConfiguration()
    ...
    var lastScrollDirection: SegMenuScrollDirection = .other
    var startingPageForScroll: Int = 0
    var didTapMenuItemToScroll: Bool = false
    
    var pagesAddedDictionary: [Int : Int] = [:]
    
    open weak var delegate: SegMenuDelegate?
    
    var tapTimer: Timer?
    
    enum SegMenuScrollDirection : Int {
        case left
        case right
        case other
    }
    ...
    public convenience init(viewControllers: [UIViewController], frame: CGRect, pageMenuOptions: [SegMenuOption]?) {
        self.init(viewControllers:viewControllers, frame:frame, options:nil)
        
        if let options = pageMenuOptions {
            configureSegMenu(options: options)
        }
     ...
     */
    public init(viewControllers: [UIViewController], frame: CGRect, configuration: SegMenuConfiguration) {
        super.init(nibName: nil, bundle: nil)
    ...
    /**
     ...
     - parameter configuration: A configuration instance for page menu
     */
    public init(viewControllers: [UIViewController], in controller: UIViewController, with configuration: SegMenuConfiguration, usingStoryboards: Bool = false) {
        super.init(nibName: nil, bundle: nil)
        self.configuration = configuration
        controllerArray = viewControllers
...
extension SegMenu {
    ...
    func moveSelectionIndicator(_ pageIndex: Int) {
        if pageIndex >= 0 && pageIndex < controllerArray.count {
            UIView.animate(withDuration: 0.15, animations: { () -> Void in
                var selectionIndicatorWidth: CGFloat = self.indicatorView.frame.width
                var selectionIndicatorX: CGFloat = 0.0
                
                if self.configuration.useMenuAsSegmentedControl {
                ...
                } else if self.configuration.autoAdjustItemWidthWithTitle {
                ...
                self.indicatorView.frame = CGRect(x: selectionIndicatorX, y: self.indicatorView.frame.origin.y, width: selectionIndicatorWidth, height: self.indicatorView.frame.height)
                
                // Switch newly selected menu item title label to selected color and old one to unselected color
                if self.menuItems.count > 0 {
                    if self.menuItems[self.lastPageIndex].titleLabel != nil && self.menuItems[self.currentPageIndex].titleLabel != nil {
                        self.menuItems[self.lastPageIndex].titleLabel!.textColor = self.configuration.unselectedMenuItemTextColor
                        self.menuItems[self.currentPageIndex].titleLabel!.textColor = self.configuration.selectedMenuItemTextColor
                    }
                }
            })
        }
    ...
    override open func viewDidLayoutSubviews() {
        ...
        if (oldCurrentOrientationIsPortrait && UIDevice.current.orientation.isLandscape) || (!oldCurrentOrientationIsPortrait && (UIDevice.current.orientation.isPortrait || UIDevice.current.orientation.isFlat)) {
            ...
            if configuration.useMenuAsSegmentedControl {
                ...
                let selectionIndicatorX : CGFloat = CGFloat(currentPageIndex) * (self.view.frame.width / CGFloat(controllerArray.count))
                let selectionIndicatorWidth: CGFloat = self.view.frame.width / CGFloat(controllerArray.count)
                indicatorView.frame = CGRect(x: selectionIndicatorX, y: self.indicatorView.frame.origin.y, width: selectionIndicatorWidth, height: indicatorView.frame.height)
                
                // Resize menu items
                var index : Int = 0
                
                for item: SegMenuItem in menuItems as [SegMenuItem] {
                ...
                indicatorView.frame =  CGRect(x: selectionIndicatorX, y: indicatorView.frame.origin.y, width: indicatorView.frame.width, height: indicatorView.frame.height)
    ...
    open func moveToPage(_ index: Int) {
        if index >= 0 && index < controllerArray.count {
            ...
            let duration : Double = Double(configuration.scrollDuration) / Double(1000)
